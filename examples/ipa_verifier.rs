...
use ipa_rust_simple_hash_batch::{proof_of_inclusion, single_IPA_proof, 
...
// currently, the input is of type single_IPA_proof
...
fn verify_single_IPA_proof(
    ...
    let G_0 = ecc_chip.
        variable_base_msm_in::<G1Affine>(
            ...
            window_bits,
         0);
    // let G_0 = L_folded.clone();
    // println!("Circuit managed to compute G_0! The value is {:?}, {:?}", bigint_to_fe::<Fr>(&G_0.x.value), bigint_to_fe::<Fr>(&G_0.y.value)  );
    let ctx = builder.main(0);
...
//                             .collect::<Vec<_>>();
//     let batched_inclusion_proof = &batched_IPA_proof.batched_proof;
...
fn main() {
    ...
    .unwrap();
    let private_inputs = test_ipa_export(6);
    // run_builder_on_inputs(verify_single_IPA_proof_hack, args, private_inputs);
    // let random_point = G1Affine::random(&mut OsRng);
     let input = test_mul_input{
         points: [G1::generator().to_affine();256],
         scalars: [Fr::random(OsRng);256],
     };
    // run_builder_on_inputs(compute_big_MSM, args, input);
    // run_builder_on_inputs(load_EC_point, args, input);
    // run_builder_on_inputs(load_EC_point, args, G1::generator().to_affine());
    // let private_inputs = Fr::random(OsRng);
    run_builder_on_inputs(verify_single_IPA_proof, args, private_inputs);
    //  run_builder_on_inputs(test_poseidon, 
    //     args,
    //     // Fr::random(OsRng)
    //     Fr::one());
}
